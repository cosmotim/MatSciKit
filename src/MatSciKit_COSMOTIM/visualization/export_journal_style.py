...
def apply_journal_style(fig: Optional[Figure] = None, ax: Optional[plt.Axes] = None) -> None:
    """
    Apply journal-style formatting to a matplotlib figure without exporting.
    
    This is a lighter version that only applies formatting without saving.
    For full export functionality, use export_journal_figure().
    
    Args:
        fig: Figure handle (defaults to current figure if None)
        ax: Axes handle (defaults to current axes if None)
    """
    if fig is None:
        fig = plt.gcf()
    if ax is None:
