...
class XRDMultilinePlotter:
    ...
    def plot_patterns(
        self,
        data_list: np.ndarray | list[np.ndarray],
        ...
        Args:
            data_list: Single array or list of arrays from XRDDataReader.
        ...
        if labels is None:
            ...
            print(
                f"Warning: Number of labels ({len(labels)}) doesn't match "
                f"number of datasets ({len(data_list)}). Using defaults."
            )
            labels = [f"Pattern {i + 1}" for i in range(len(data_list))]
        ...
        for i, data_item in enumerate(data_list):
            ...
            # Plot Main Intensity
            ax.plot(
                two_theta, intensity + y_offset, "-", color=self.colors[color_idx], label=labels[i]
            )
...
def main():
    ...
    # Create a name list from filenames in the XRD_data directory
    ...
    if data_filenames:
        ...
        if sample_data is not None:
            ...
            fig2 = plotter.plot_patterns(sample_data, markers=peak_positions)
