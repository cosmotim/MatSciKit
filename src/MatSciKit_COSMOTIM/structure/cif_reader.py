    ...
    while i < len(lines):
        ...
        if line == "loop_":
            ...
            while i < len(lines):
                ...
                if (
                    not dline
                    or dline.startswith("_")
                    or dline.startswith("loop_")
                    or dline.startswith("#")
                ):
                    break
                parts = dline.split()
...
def read(path: str | Path, use_pymatgen: bool | None = None) -> dict:
    """
    ...
    if should_use_pmg and _PYMATGEN_AVAILABLE:
        ...
        raise ImportError(
            "pymatgen requested but not installed. Install with: pip install pymatgen"
        )
        ...
        total_mass_amu = sum(a["occupancy"] * ATOMIC_MASSES.get(a["symbol"], 0) for a in atoms)
...
def get_material_params(
    path: str | Path, use_pymatgen: bool | None = None
